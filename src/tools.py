...
def load_tile_size_image(path):
	...
	try:
		...
		img = pg.transform.scale(pg.image.load('src/assets/texture/tile/unknown.png').convert_alpha(), TILE_S)
	return img
...
def load_item_icon(path):
	...
	try:
		...
		img = pg.transform.scale(pg.image.load('src/assets/texture/tile/unknown.png').convert_alpha(), tuple(map(int, (TILE_W / 1.3, TILE_H / 1.3))))
	return img
