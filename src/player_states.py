...
class Falling(YMovementState):
	@classmethod
	def on_enter(cls, player):
		player.velocity_y = lambda dt: math.log(player.fall_time * dt)
		player.falling_time = 1

	def update(self, dt):
		self.velocity_y = lambda dt: math.log(self.fall_time * dt)
		self.fall_time += dt
...
class Jumping(YMovementState):
	@classmethod
	def on_enter(cls, player):
		player.velocity_y = lambda dt: -math.log(player.jump_time * dt)
	...
	def update(self, dt):
		self.velocity_y = lambda dt: ((-self.jump_time * (self.jump_time - self.jump_gravity)) / (
			(self.jump_gravity / 2) ** 2)) * self.jump_height
